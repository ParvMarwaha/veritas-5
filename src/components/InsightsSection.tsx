  ...
  return (
    <section className="relative w-full bg-[#111] text-white py-20 md:py-28 overflow-hidden border-t border-white/5">
      ...
      >
        ...
        {/* Header */}
        <motion.div variants={itemVariants} className="mb-16 flex flex-col md:flex-row md:items-start justify-between gap-8">
          <div>
          ...
          <div className="w-full md:w-auto flex-shrink-0 mt-2">
            <AnimatedButton variant="text" className="border border-white/20 px-8 py-4">
              View All Insights
            </AnimatedButton>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-px bg-white/10 border border-white/10">
          ...
          <motion.div variants={itemVariants} className="lg:col-span-7 relative bg-[#111] p-8 md:p-12 min-h-[500px] flex flex-col justify-end group cursor-pointer overflow-hidden">
            ...
            <div className="relative z-10 w-full flex flex-col">
              <div className="flex justify-between items-start w-full mb-8">
                <span className="text-white/60 font-mono text-[15px] md:text-[15px] uppercase tracking-widest bg-white/10 px-4 py-2 backdrop-blur-md">
                  {insights[activeIndex].tag}
                </span>
                <span className="text-white/60 font-mono text-[15px]">
                  {insights[activeIndex].num}
                </span>
              </div>
              <h3 className="text-[20px] md:text-[20px] font-sans tracking-tight leading-[1.2] mb-8 group-hover:text-[#D02717] transition-colors duration-500">
          ...
          <div className="lg:col-span-5 grid grid-rows-4 gap-px bg-white/10">
            {insights.map((item, index) => (
              <motion.div 
                ...
                className={`relative bg-[#111] p-8 md:p-12 flex flex-col justify-between group cursor-pointer transition-all duration-700 ${activeIndex === index ? 'bg-white/[0.05]' : 'hover:bg-white/[0.02]'}`}
              >
                <div className="flex justify-between items-center w-full mb-6">
                  <span className={`font-mono text-[15px] md:text-[15px] uppercase tracking-widest transition-colors duration-500 ${activeIndex === index ? 'text-white' : 'text-white/40 group-hover:text-white'}`}>
                    {item.tag}
                  </span>
                  <span className={`font-mono text-[15px] transition-colors duration-500 ${activeIndex === index ? 'text-[#D02717]' : 'text-white/20 group-hover:text-[#D02717]'}`}>
                    {item.num}
                  </span>
                </div>
                <div className="flex justify-between items-end w-full mt-auto">
                  <h4 className={`text-[20px] md:text-[20px] font-sans tracking-tight leading-[1.2] max-w-[80%] transition-transform duration-500 ${activeIndex === index ? 'text-white translate-x-2' : 'text-white/70 group-hover:translate-x-2 group-hover:text-white/90'}`}>
                    {item.title}
                  </h4>
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-500 ${activeIndex === index ? 'bg-[#D02717] border-[#D02717]' : 'border-white/10 group-hover:bg-[#D02717] group-hover:border-[#D02717]'}`}>
